...
def main():
    ...
    if not config_path:
        out = {
            ...
            "skill_tracker": str(sibling(skill_root, "mmkc-keyword-tracker-skill")),
    ...
    scout = ws / "mmkc-keyword-scout-skill"
    if not scout.is_dir():
        scout = skill_root
    ...
    result = {
        ...
        "threshold_presets_doc": "mmkc-keyword-scout-skill/references/threshold-presets.md",
        "skill_scout": str(scout),
        "skill_tracker": str(ws / "mmkc-keyword-tracker-skill"),
        ...
        "scripts": {
            ...
            "compare_versions": str(ws / "mmkc-keyword-tracker-skill" / "scripts" / "compare_versions.py"),
        },
    ...
    if needs or not cfg.get("output_dir"):
        result["first_time_guide"] = {
            "doc": "mmkc-keyword-scout-skill/references/first-time-setup.md",
        ...
        if result["needs_output_dir"]:
            guide.append({
                "id": "output_dir",
                "required": True,
                "prompt": "请用户提供「报告输出根目录」完整路径（所有 ASIN 共用，只配置一次）。禁止擅自填写他人路径。详见 mmkc-keyword-scout-skill/references/first-time-setup.md",
